...
const assets = {
  "/index.mjs": {
    ...
    "etag": "\"7ff46-eWuJVpeVaQXO4Us4bSS5gBKnH4E\"",
    "mtime": "2026-03-17T14:25:51.166Z",
    "size": 524102,
    "path": "index.mjs"
  },
  "/index.mjs.map": {
    "type": "application/json",
    "etag": "\"1fd9b9-33xWCpLLuTgKwcGTj0w9JUGjvYQ\"",
    "mtime": "2026-03-17T14:25:51.167Z",
    "size": 2087353,
    "path": "index.mjs.map"
  }
};
...
class ProjectService {
  ...
  async getAsync(project_id) {
    const { data, error } = await useSupabase().from("projects").select("*").eq("id", project_id).single();
    if (error) throw createError({ statusCode: 404, message: "Projet introuvable" });
    return data;
  }
  async createProject(link) {
    var _a, _b;
    const match = link.match(/github\.com\/([^/]+\/[^/]+)/);
    const ghData = await $fetch(`https://api.github.com/repos/${match[1]}`);
    const project = {
      title: ghData.name,
      description: (_a = ghData.description) != null ? _a : "",
      link: ghData.html_url,
      language: (_b = ghData.language) != null ? _b : "",
      is_featured: false
    };
    const { data, error } = await useSupabase().from("projects").insert(project).select().single();
    if (error) throw createError({ statusCode: 500, message: error.message });
    return data;
  }
}
